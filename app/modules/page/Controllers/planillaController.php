 ...
 */
		...
		<td><div align="center"><strong>' . $this->formato_numero($totales['seguridad_social']) . '</strong></div></td>
		<td><div align="center"><strong>' . $this->formato_numero($totales['seguro_educativo']) . '</strong></div></td>
		<td><div align="center"  id="total_viaticos"><strong>' . $this->formato_numero($totales['viaticos']) . '</strong></div></td>
		<td><div align="center" id="total_prestamos"><strong>' . $this->formato_numero($totales['prestamos']) . '</strong></div></td>
		<td><div align="center" id="total_prestamos2"><strong>' . $this->formato_numero($totales['prestamos2']) . '</strong></div></td>
		<td><div align="center" id="total_decimo"><strong>' . $this->formato_numero($totales['decimo']) . '</strong></div></td>
		<td><div align="center" id="total_neta1"><strong>' . $this->formato_numero($totales['neta']) . '</strong></div></td>
		</tr>';
 ...
 */
	...
	public function horasnormalesAction()
	{
		...
		// 5 dias anteriores de la quincena actual para agregar los pendientes
		...
		foreach ($cedulas as $value) {
			...
			foreach ($cincoAnteriores as $keyDia => $diaAnterior) {
				$diaPend = $keyDia + 1;

				$tabla .= '<td align="center">';
				$horas = $planillaHorasModel->getList(" planilla = '$id' AND fecha = '$diaAnterior' AND cedula ='$cedula'  AND tipo='$tipo' AND pendiente = 'pendiente$diaPend' ", "")[0];
				...
				<div align="left" class="nowrap">
					...
					<div class="enlinea">
					<input 
					name="horas_' . $i . '_' . $diaPend . '" 
					type="text" 
					id="horas_' . $i . '_' . $diaPend . '" 
					class="v form-control pendiente" 
					value="' . $horas->horas . '" 
					onkeyup="guardar_hora_pendiente(' . "'$i'" . ',' . "'$diaPend'" . ')" 
					onchange="guardar_hora_pendiente(' . "'$i'" . ',' . "'$diaPend'" . ')" />
					</div>
				  </div>
				  ...
				  <div align="left" class="nowrap">
					...
					<div class="enlinea"> 
					<select 
					name="loc_' . $i . '_' . $diaPend . '" 
					id="loc_' . $i . '_' . $diaPend . '" 
					class="v form-select pendiente" 
					onchange="guardar_hora_pendiente(' . "'$i'" . ',' . "'$diaPend'" . ')" 
					title="' . $horas->loc . '">
				...
				<input id="fechapend_' . $i . '_' . $diaPend . '" name="fechapend_' . $i . '_' . $diaPend . '" type="hidden" value="' . $diaAnterior . '" />
	...
	public function guardarhoraspendientesAction()
	{
		ini_set("display_errors", 0);
		header('Content-Type:application/json');
		$this->setLayout('blanco');
		...
		$tipo = $this->_getSanitizedParam("tipo");
		$general = $this->_getSanitizedParam("general");
		$pendiente = $this->_getSanitizedParam("pendiente");
		...
		if (count($horasConsulta) > 0) {
			...
			if ($general == 0) {
				$planillaHorasModel->editField($id, 'horas', $horas);
				$planillaHorasModel->editField($id, 'loc', $loc);
				$planillaHorasModel->editField($id, 'pendiente', $pendiente);
			}
			if ($general == 1) {
				$planillaHorasModel->editField($id, 'horas', $horas);
				$planillaHorasModel->editField($id, 'general', $loc);
			...
			if ($general == 0) {
				...
				$data['planilla'] = $this->_getSanitizedParam("planilla");
				$data['tipo'] = $this->_getSanitizedParam("tipo");
				$data['pendiente'] = $this->_getSanitizedParam("pendiente");


				$res = $planillaHorasModel->insertpendientes($data);
			}
			if ($general == 1) {
				$data = [];
				$data['fecha'] = $this->_getSanitizedParam("fecha");
	...
	public function consultanetaAction()
	{
		ini_set("display_errors", 0);
		header('Content-Type:application/json');
		$this->setLayout('blanco');
		...
		$cedula = $this->_getSanitizedParam("cedula");
		$planilla = $this->_getSanitizedParam("planilla");
		$viaticos = $this->_getSanitizedParam("viaticos");
		$prestamos = $this->_getSanitizedParam("prestamos");
		$prestamos2 = $this->_getSanitizedParam("prestamos2");
		$decimo = $this->_getSanitizedParam("decimo");
		$neta = $this->_getSanitizedParam("neta");
		...
		$horasConsulta = $planillaTotalesModel->getList(" planilla = '$planilla'  AND cedula = '$cedula' ", "");
		...
		if (count($horasConsulta) > 0) {
			...
			$planillaTotalesModel->editField($id, 'viaticos', $viaticos);
			$planillaTotalesModel->editField($id, 'prestamos', $prestamos);
			$planillaTotalesModel->editField($id, 'prestamos_financiera', $prestamos2);
			$planillaTotalesModel->editField($id, 'decimo', $decimo);
			$planillaTotalesModel->editField($id, 'neta', $neta);
		} else {
			$data = [];
			$data['cedula'] = $cedula;
			...
			$data['prestamos2'] = $prestamos2;
			$data['decimo'] = $decimo;
			$data['neta'] = $neta;
			$res = $planillaTotalesModel->insert2($data);
		}
		$respuesta['id'] = $res;
		/*$respuesta['loc'] = $fecha1;
		$respuesta['fecha1'] = $fecha2;
 ...
 */
			...
			<td><div align="left"><strong>' . $value->nombre1 . '</strong></div></td>
			
			<td><div align="left"><strong>CEDULA</strong></div></td>
			<td><div align="left"><strong>' . $cedula . '</strong></div></td>
			<td><div align="right"><strong>' . $general->general . '</strong></div></td>
			<td><div align="left"><strong>MÉTODO DE PAGO</strong></div></td>
			<td><div align="left"><strong>' . $metodoPago . '</strong></div></td>
	...
	public function exportarreciboplanoAction()
	{
		...
		$empresaId = $planilla->empresa;
		$empresa = $empresaModel->getById($empresaId);
		$cedulas = $planillaAsignacionModel->getListCedulas(" planilla= '$id' AND hoja_vida.metodo_pago = '2' " . $filtro, "nombre1 ASC");
		$parametros = $parametrosModel->getById(1);
 ...
 */
		 ...
		 echo '</pre>'; */
			...
			<td><div align="left"><strong>' . $cedula . '</strong></div></td>
			<td><div align="right"><strong>' . $general->general . '</strong></div></td>
			<td><div align="left"><strong>METODO DE PAGO</strong></div></td>
			<td><div align="left"><strong>' . $metodoPago . '</strong></div></td>
 ...
 */
	...
	public function obtenerCincoDiasAnteriores($fecha)
	{
		$fechaActual = new DateTime($fecha);
		$ultimoDiaMesAnterior = new DateTime("last day of " . $fechaActual->format("Y-m") . " -1 month");
		$dias = [];

		if ($fechaActual->format("d") == "01") {
			$primerDiaMesAnterior = new DateTime($ultimoDiaMesAnterior->format("Y-m-01"));
			$intervalo = new DateInterval("P1D");

			$diasAMostrar = min(5, $primerDiaMesAnterior->format("t")); // Obtener el mínimo entre 5 y los días del mes anterior

			for ($i = 0; $i < $diasAMostrar; $i++) {
				$dias[] = $ultimoDiaMesAnterior->format("Y-m-d");
				$ultimoDiaMesAnterior->sub($intervalo);
			}
			...
			while ($primerDiaRango <= $ultimoDiaRango) {
				$dias[] = $primerDiaRango->format("Y-m-d");
				$primerDiaRango->add($intervalo);
			}
		}
